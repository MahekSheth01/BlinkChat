...
export async function sendFriendRequest(req,res){
    try{
        ...
        //checck if users are already friends
        ...
        if(existingRequest){
            return res.status(400).json({message:"You have already sent a friend request to this"})
        }
...
export async function acceptFriendRequest(req,res){
    try{
        ...
        //$addToSet : adds elements to an array if they do not exists.
...
export async function getFriendRequests(req,res){
    try{
        const incomingReqs=await FriendRequest.find({
            ...
            status:"pending",
        }).populate("sender","fullName profilePc nativeLanguage learningLanguage")
